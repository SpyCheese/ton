...
ninja storage-daemon storage-daemon-cli fift func tolk tonlib tonlibjson tonlib-cli \
      ...
      adnl-proxy create-state emulator proxy-liteserver dht-ping-servers dht-resolve tps-counter
      test $? -eq 0 || { echo "Can't compile ton"; exit 1; }
fi
...
if [ "$with_artifacts" = true ]; then
  ...
  cp build/storage/storage-daemon/storage-daemon build/storage/storage-daemon/storage-daemon-cli \
     ...
     build/dht/dht-ping-servers build/dht/dht-resolve build/tps-counter/tps-counter \
     artifacts
