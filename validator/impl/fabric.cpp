...
td::actor::Task<> run_accept_block_query(BlockIdExt id, td::Ref<BlockData> data, std::vector<BlockIdExt> prev,
  ...
  run_accept_block_query(id, std::move(data), std::move(prev), std::move(validator_set), std::move(signatures),
                         send_broadcast_mode, apply, std::move(manager), std::move(promise));
