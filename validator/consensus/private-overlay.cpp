...
class PrivateOverlayImpl : public runtime::SpawnsWith<ConsensusBus>, public runtime::ConnectsTo<ConsensusBus> {
 public:
  ...
  template <>
  void handle(runtime::BusHandle<ConsensusBus>, std::shared_ptr<const ConsensusBus::OutgoingProtocolMessage> message) {
    ...
    if (message->recipient) {
      ...
      send_to_peer(owning_bus()->validator_set[message->recipient->value()].adnl_id);
    } else {
 ...
 private:
  ...
  void on_overlay_message(adnl::AdnlNodeIdShort src_adnl_id, td::BufferSlice data) {
    ...
    owning_bus().publish<ConsensusBus::IncomingProtocolMessage>(peer.idx, std::move(data));
  }
  ...
  void on_overlay_broadcast(PublicKeyHash src, td::BufferSlice data) {
    ...
    auto& bus = *owning_bus();
    auto maybe_candidate = Candidate::deserialize(std::move(data), short_id_to_peer_.at(src), bus);
    ...
    owning_bus().publish<ConsensusBus::CandidateReceived>(maybe_candidate.move_as_ok());
  }
