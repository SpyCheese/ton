...
class TestOverlay : public td::actor::Actor {
 public:
  void register_node(int idx, td::actor::ActorId<TestOverlayNode> node) {
    if (nodes_.size() <= idx) {
      nodes_.resize(idx + 1);
    }
  ...
  td::actor::Task<> send_message(PeerValidator src, int dst_idx, td::BufferSlice message);
  td::actor::Task<> send_candidate(PeerValidator src, int dst_idx, RawCandidateRef candidate);

 private:
  std::vector<std::vector<td::actor::ActorId<TestOverlayNode>>> nodes_;

  td::actor::Task<> before_receive(int src_idx, int dst_idx) {
...
td::actor::Task<> TestOverlay::send_message(PeerValidator src, int dst_idx, td::BufferSlice message) {
...
td::actor::Task<> TestOverlay::send_candidate(PeerValidator src, int dst_idx, RawCandidateRef candidate) {
  co_await before_receive(src.idx.value(), dst_idx);
  for (const auto &instance : nodes_[dst_idx]) {
