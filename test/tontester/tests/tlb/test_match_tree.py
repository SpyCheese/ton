...
) -> tuple[MatchTree, dict[str | None, ResolvedConstructor]]:
    _, user_types = analyze_text(text)
    ts = {t.name: t for t in user_types}
    t = ts[type_name]
    assert t.match_tree is not None
    return t.match_tree, {c.name: c for c in t.constructors}


def analyze_cons(text: str, type_name: str) -> tuple[MatchTree, list[ResolvedConstructor]]:
    """Like analyze_type but returns constructors as list (for anonymous constructors)."""
    _, user_types = analyze_text(text)
    ts = {t.name: t for t in user_types}
    t = ts[type_name]
    assert t.match_tree is not None
    return t.match_tree, t.constructors
...
def EQ(pos: int, val: int) -> CheckConstraint:
    return CheckConstraint(op=CompareOp.EQ, left=NatTypeArg(position=pos), right=NatLiteral(val))


def LE(pos: int, val: int) -> CheckConstraint:
    return CheckConstraint(op=CompareOp.LE, left=NatTypeArg(position=pos), right=NatLiteral(val))
...
class TestMultiConstructorBits:
    def test_three_constructors(self):
        tree, c = analyze_type("a$00 = T; b$01 = T; c$1 = T;", "T")
    ...
    def test_four_balanced(self):
        tree, c = analyze_type("a$00 = T; b$01 = T; c$10 = T; d$11 = T;", "T")
        assert tree == MatchBit(
            zero=MatchBit(zero=L(c["a"]), one=L(c["b"])),
    ...
    def test_unbalanced_prefix_lengths(self):
        tree, c = analyze_type("a$0 = T; b$10 = T; c$11 = T;", "T")
        assert tree == MatchBit(
            zero=L(c["a"]),
    ...
    def test_three_with_common_prefix(self):
        tree, c = analyze_type("a$100 = T; b$101 = T; c$11 = T;", "T")
    ...
    def test_inmsg_pattern(self):
        tree, c = analyze_type("a$000 = T; b$010 = T; c$011 = T; d$100 = T; e$101 = T;", "T")
        assert tree == MatchBit(
            zero=MatchBit(
                zero=L(c["a"]),
                one=MatchBit(zero=L(c["b"]), one=L(c["c"])),
...
class TestConstraintDispatch:
    ...
    def test_two_constants(self):
        tree, cons = analyze_cons("_ x:uint32 = P 0; _ x:uint64 = P 1;", "P")
        c0, c1 = cons[0], cons[1]
        assert tree == MatchConstraint(condition=LE(0, 0), if_true=L(c0), if_false=L(c1))
    ...
    def test_three_constants(self):
        ...
        assert tree == MatchConstraint(
            condition=LE(0, 0),
            if_true=L(c0),
            if_false=MatchConstraint(condition=LE(0, 1), if_true=L(c1), if_false=L(c2)),
        )
    ...
    def test_five_constants(self):
        ...
        assert tree == MatchConstraint(
            condition=LE(0, 1),
            if_true=MatchConstraint(condition=LE(0, 0), if_true=L(c0), if_false=L(c1)),
            if_false=MatchConstraint(
                condition=LE(0, 2),
                if_true=L(c2),
                if_false=MatchConstraint(condition=LE(0, 3), if_true=L(c3), if_false=L(c4)),
            ),
        )
    ...
    def test_split_on_second_param(self):
        ...
        c0, c1 = cons[0], cons[1]
        assert tree == MatchConstraint(condition=LE(1, 0), if_true=L(c0), if_false=L(c1))
