...
class SampleActor : public SpawnsWith<MainBus>, public ConnectsTo<MainBus> {
 public:
  ...
  void start_up() {
    ...
    owning_bus().publish(std::make_shared<MainBus::SampleEvent>(43));
  }

  template <>
  void handle(BusHandle<MainBus> bus, std::shared_ptr<MainBus::SampleEvent const> event) {
 ...
 public:
  ...
  void start_up() {
    ...
    for (auto& child : children_) {
      child.publish(std::make_shared<Level1Bus::StopRequested>());
    }
    stop();
  }
...
class Level1Controller : public SpawnsWith<Level1Bus>, public ConnectsTo<Level1Bus, Level2Bus> {
 public:
  ...
  template <>
  void handle(BusHandle<Level1Bus> bus, std::shared_ptr<Level1Bus::StopRequested const>) {
    if (child_) {
      child_.publish(std::make_shared<Level2Bus::StopRequested>());
    }
    stop();
  }
...
class Level2Worker : public SpawnsWith<Level2Bus>, public ConnectsTo<Level2Bus> {
 public:
  ...
  template <>
  void handle(BusHandle<Level2Bus> bus, std::shared_ptr<Level2Bus::StopRequested const>) {
    stop();
  }
};
...
class ParentBusActor : public SpawnsWith<ParentBus>, public ConnectsTo<ParentBus> {
 public:
  ...
  void start_up() {  // 1st
    ...
    owning_bus().publish(std::make_shared<ParentBus::ParentEvent>(100));
  }

  template <>
  void handle(BusHandle<ParentBus> bus, std::shared_ptr<ParentBus::ParentEvent2 const> event) {  // 4th
    check_bus(bus);
    EXPECT_EQ(event->value, 102);
    g_ran_4th_in_parent = true;

    owning_bus().publish(std::make_shared<ParentBus::ParentEvent3>(103));
...
class ChildBusActor : public SpawnsWith<ChildBus>, public ConnectsTo<ChildBus> {
 public:
  ...
  template <>
  void handle(BusHandle<ChildBus> bus, std::shared_ptr<ParentBus::ParentEvent const> event) {  // 2nd
    check_bus(bus);
    EXPECT_EQ(event->value, 100);
    g_ran_2nd = true;

    owning_bus().publish(std::make_shared<ChildBus::ChildEvent>(101));
  }

  template <>
  void handle(BusHandle<ChildBus> bus, std::shared_ptr<ChildBus::ChildEvent const> event) {  // 3rd
    check_bus(bus);
    EXPECT_EQ(event->value, 101);
    g_ran_3rd = true;

    owning_bus().publish(std::make_shared<ParentBus::ParentEvent2>(102));
  }

  template <>
  void handle(BusHandle<ChildBus> bus, std::shared_ptr<ChildBus::ParentEvent2 const> event) {  // 4th
    check_bus(bus);
    EXPECT_EQ(event->value, 102);
  ...
  template <>
  void handle(BusHandle<ChildBus> bus, std::shared_ptr<ParentBus::ParentEvent3 const> event) {  // 5th
    check_bus(bus);
